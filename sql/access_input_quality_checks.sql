...
WITH pod_stats AS (
    SELECT
        ...
        , COUNTIF(source_kind = 'pod_access_history') AS history_row_count
    FROM
...
, checks AS (
    ...
    SELECT 'access_input_sales_row_count_zero', 'ERROR', 'access_input_sales', IF(COUNT(*) = 0, 1, 0), NULL
    FROM `{{ project_id }}.{{ source_dataset }}.access_input_sales`

    UNION ALL

    SELECT 'access_input_store_detail_row_count_zero', 'ERROR', 'access_input_store_detail', IF(COUNT(*) = 0, 1, 0), NULL
    FROM `{{ project_id }}.{{ source_dataset }}.access_input_store_detail`

    UNION ALL

    SELECT 'access_input_pod_sales_row_count_zero', 'ERROR', 'access_input_pod_sales', IF(COUNT(*) = 0, 1, 0), NULL
    FROM `{{ project_id }}.{{ source_dataset }}.access_input_pod_sales`

    UNION ALL

    SELECT
        'access_input_sales_accounting_month_mismatch', 'ERROR', 'access_input_sales',
    ...
    SELECT
        'access_input_sales_pod_mixed', 'ERROR', 'access_input_sales',
    ...
    SELECT
        'access_input_store_detail_pod_mixed', 'ERROR', 'access_input_store_detail',
    ...
    SELECT
        'access_input_store_detail_legacy_store_name_remaining', 'ERROR', 'access_input_store_detail',
    ...
    SELECT
        'access_input_pod_sales_unknown_source_kind', 'ERROR', 'access_input_pod_sales',
    ...
    SELECT
        'access_input_pod_sales_target_month_mismatch', 'ERROR', 'access_input_pod_sales',
    ...
    SELECT
        'access_input_pod_sales_202602_row_count', 'ERROR', 'access_input_pod_sales',
    ...
    SELECT
        'access_input_pod_sales_202602_quantity_total', 'ERROR', 'access_input_pod_sales',
    ...
    SELECT
        'access_input_pod_sales_202602_sales_amount_total', 'ERROR', 'access_input_pod_sales',
    ...
    SELECT
        'access_input_pod_sales_202602_manufacturing_cost_total', 'ERROR', 'access_input_pod_sales',
    ...
    SELECT
        'access_input_pod_sales_202602_amazon_row_count', 'ERROR', 'access_input_pod_sales',
        IF(v_accounting_month = '202602' AND history_row_count = 0 AND amazon_row_count != 4, 1, 0),
        IF(v_accounting_month = '202602', TO_JSON_STRING(STRUCT(4 AS expected, amazon_row_count AS actual, history_row_count AS seeded_history_rows)), NULL)
    FROM pod_stats

    UNION ALL

    SELECT
        'access_input_pod_sales_202602_pf_row_count', 'ERROR', 'access_input_pod_sales',
        IF(v_accounting_month = '202602' AND history_row_count = 0 AND pf_row_count != 15, 1, 0),
        IF(v_accounting_month = '202602', TO_JSON_STRING(STRUCT(15 AS expected, pf_row_count AS actual, history_row_count AS seeded_history_rows)), NULL)
    FROM pod_stats

    UNION ALL

    SELECT
        'access_input_sales_amazon_941_return_adjustment_remaining', 'ERROR', 'access_input_sales',
        COUNTIF(
    ...
    SELECT
        'access_input_store_detail_202506_manual_adjustment_count', 'ERROR', 'access_input_store_detail',
        IF(
