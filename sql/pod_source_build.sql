...
WITH amazon AS (
  SELECT
    ...
    REGEXP_REPLACE(dl_month, r'[^0-9]', '') AS dl_month,
    REGEXP_REPLACE(sales_month, r'[^0-9]', '') AS sales_month,
...
pf AS (
  SELECT
    ...
    FORMAT_DATE('%Y%m', DATE_SUB(PARSE_DATE('%Y%m', v_target_month), INTERVAL 1 MONTH)) AS dl_month,
  ...
  WHERE target_month = v_target_month
    AND COALESCE(SAFE_CAST(REGEXP_REPLACE(quantity, r'[^0-9\-]', '') AS INT64), 0) != 0
)
SELECT * FROM amazon
